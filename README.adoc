...
* I fixed the many visual bugs (horizon moving and rotating wackily, everything getting overexposed, camera moving lower inside the vehicle, objects disappearing) when the first person vehicle camera would roll.
* I fixed most missing objects creating holes in the ground and missing water tiles that could be seen from a high altitude.
...
* I fixed firing rockets on foot while moving fast (like being atop a fast moving vehicle) so that it's safe to fire rockets in any direction while using my remote controlled car cheat and I fixed how fast rockets travel.
* I fixed things that would be animated at the incorrect speed (like water or weather effects) when the game isn't playing at 1x speed and 50 fps.
...
* I fixed many things related to standing on top of a vehicle so that the ped can stand on a moving vehicle without moving around or getting hurt.
* Popped wheels have friction, but after fixing the vehicle popped wheels would have zero friction. I fixed that.
...
* Airline planes now have collisions enabled, you can't just go through them anymore, they can push you, you can stand on them or land on them.
* They also have their motion smoothed so they fly more smoothly and realistically.
* I disabled the way that the camera would suddenly pitch down when the vehicle would pitch straight down (common when doing flips).
...
* I made jumping on foot 25% higher and 25% faster (in terms of forward speed) by default.
* You can now exit cars at any speed instantly (done in `CVehicle::CanPedExitCar()` and the other two functions below it) by just getting out and closing the door (unless you're going over 80 km/h in which case you roll out), because it's funny.
* I changed the camera transition when exiting a car so that it doesn't make you look at the car but rather makes you look in the same direction you were already looking.
...
Since I added instant car exit (not a cheat, it's there by default) it's even more practical as now you can bail out quickly and detonate.
...
==== G46 / G47 - Teleport yourself and your vehicle up 40 metres / down 6 metres.

Useful for taking flight easily or getting out of a bad place. If you're outside of a vehicle but have a remote controlled vehicle (G22) then both the car and you as a ped will be teleported up. Teleporting down while on land allows you to fly underground and underwater.

==== G48 - Regenerative health

Slowly regenerate your body's health according to the Gaussian function. When you get hit the timer resets to the middle of the upside down Gaussian function so that it progressively transitions back to full health. Regenerative health gets disabled if you use G44, so if you do you have to use G48 to turn it back on. If you die while G48 is on you will spawn with almost zero health but it will regenerate.
...
Peds get guns and attack cops and paramedics.
...
The most essential combination of cheats, flip your burning car with G41 and fix it with G44. If you're in a bad place you can also use G46 to teleport yourself way up and with G35's flips you can orient yourself in a direction then use `Num0` to boost yourself there.
...
G36 and G30 make all cars drift around, and G35 enables jumping, so that when you drift towards an oncoming car you can just jump over it to maintain your momentum. Instead of jumping you could also use G06 or G32 to make sure no car gets in your way. G35 also allows reorienting your car (with `Num6` + `A` or `D`) to fix your heading quickly and `Num0` can give you the sudden thrust needed to adjust your trajectory, so that you're not just drifting but also kind of handling your car like a rocket.
...
==== G32 + G01 + G03 - Ride fast unimpeded while everything explodes around you.

G32 activates vehicle antigravity, G01 maximises the number of vehicles on the road, and if you're on a bike, G03 prevents you from falling off. Take a motorbike and go fast without worrying about traffic because traffic is flying off and exploding in front of you. Very relaxing.

==== G13 + G48 + G06 + G53 + G520 - Survive the madness

G13 makes the peds armed and violent towards each other and you, G48 is regenerative health so that you have to manage your health (avoid fights when close to death to recover), G06 removes cars so that you will be alone against the peds with no easy way out, G53 removes ambulances so that they won't bother you (maybe this could be replaced by G15 to make peds attack paramedics instead, but then they won't be as aggressive towards you), and G520 means the cops won't bother you as you kill your way out of this mess.
