...
++++
<p align="center">
  <img width="500" src="./img/logo_edited.png">
</p>
++++
...
==== G35 - Tony Hawk Pro Skater mode

I really thought to myself "driving normally is boring, I'd rather have Tony Hawk Pro Skater but with cars instead of a skateboard". Requires the use of a num keypad, so laptop users probably can't enjoy that one.
...
You'll enter the nearest car as passenger. Seems to only work if your vehicle is remote controlled (G22).
...
Almost no car appears on the road, peds still appear normally.
...
Peds (pedestrians) get weapons and attack you and cars drive dangerously. Cannot be toggled off.
...
Adding G514 (4 star wanted level) is an interesting variant because FBI cars will drift towards you at high speed, and with G35 you can jump and/or tilt your car backwards so that when FBI cars hit you it throws you high in the air, and then by doing flips (also with G35) do cool tricks to land on your wheels.
...
If you want to compile it yourself you'll need to put my library https://github.com/Photosounder/rouziclib[rouziclib] somewhere and make sure the compiler finds it. For me it's in `D:\VC_libs\include`, you can see that if you open `build/reVC.vcxproj` with a text editor.
